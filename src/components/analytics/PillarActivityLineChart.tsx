...
export function PillarActivityLineChart({ dailyCounts, period }: PillarActivityLineChartProps) {
  const { width } = useWindowDimensions();

  // Screen padding (xl * 2) + card padding (lg * 2)
  const chartWidth = width - spacing.xl * 2 - spacing.lg * 2;

  const { lineDataSets, maxCount } = useMemo(() => {
    if (dailyCounts.length === 0) {
      return { lineDataSets: null, maxCount: 5 };
    }
    ...
    // Compute max value across all pillar totals; floor at 5 to prevent flat chart on zero data
    const allValues = [...pillarTotals[1], ...pillarTotals[2], ...pillarTotals[3]];
    const computedMax = Math.max(...allValues, 5);
    ...
    return { lineDataSets: dataSets, maxCount: computedMax };
  }, [dailyCounts, period]);
